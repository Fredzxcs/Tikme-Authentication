...
class ForgotSetupQuestionsView(views.APIView):
    ...
    def get(self, request, uidb64, token):
            try:
                ...
                if user.status.status_name == "Permanently Locked":
                    return JsonResponse({"error": "Your account is permanently locked. Contact support."}, status=403)

                # ✅ Handle temporary lock (Check both status and cache)
                locked_until = cache.get(f"locked_until_{user.id}")  # Get locked time from cache
                ...
                if locked_until and now() < locked_until:
                    ...
                    # ✅ If API request, return lock message
                    if "application/json" in request.META.get("HTTP_ACCEPT", ""):
                        return JsonResponse({
                            "error": f"Too many failed attempts. Try again in {time_remaining} minutes.",
                ...
                questions = {
                    f"question_{i+1}": answer.question.question_text for i, answer in enumerate(security_answers)
                }
                questions.update({
                    f"question_{i+1}_id": answer.question.id for i, answer in enumerate(security_answers)
                })

                # ✅ If HTML is requested, pass locked status to template
    ...
    def post(self, request, uidb64, token):
        try:
            ...
            # ✅ Check if the account is permanently locked
            if user.status.status_name == "Permanently Locked":
                return Response({"error": "Your account is permanently locked. Contact support."}, status=status.HTTP_403_FORBIDDEN)

            # ✅ Handle temporary lock
            locked_until = cache.get(f"locked_until_{user.id}")
            if user.status.status_name == "Temporarily Locked" and locked_until and now() < locked_until:
                time_remaining = (locked_until - now()).seconds // 60
                return Response({"error": f"Too many failed attempts. Try again in {time_remaining} minutes."}, status=status.HTTP_403_FORBIDDEN)

            # ✅ Retrieve security answers from request
            answers_data = request.data.get("answers", [])
            if not answers_data or len(answers_data) != 3:
                return Response({"error": "All three security questions must be answered."}, status=status.HTTP_400_BAD_REQUEST)

            # ✅ Cache key for tracking failed attempts
            attempts_key = f"forgot_password_attempts_{user.id}"
            ...
            if attempts >= PERMANENT_LOCK_THRESHOLD:
                ...
                return Response({"error": "Your account is permanently locked. Please contact support."}, status=status.HTTP_403_FORBIDDEN)
            ...
            if attempts >= MAX_ATTEMPTS:
                ...
                return Response({"error": "Too many failed attempts. Your account is temporarily locked."}, status=status.HTTP_403_FORBIDDEN)

            # ✅ Validate each security answer
            for ans_data in answers_data:
                question_id = ans_data.get("question_id")
                answer_text = ans_data.get("answer", "").strip().lower()

                stored_answer = SecurityAnswer.objects.filter(user=user, question_id=question_id).values_list("answer", flat=True).first()

                if not stored_answer or stored_answer.lower() != answer_text:
                    cache.set(attempts_key, attempts + 1, timeout=TEMPORARY_LOCK_TIME)
                    logger.warning(f"Security answer mismatch for user {user.email}, Question ID: {question_id}, Submitted Answer: {answer_text}")
