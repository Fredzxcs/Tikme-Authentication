...
class SetupAccountView(views.APIView):
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if user.status.status_name == "Active":  # Ensure account setup is incomplete
            ...
            if request.META.get('HTTP_ACCEPT', '').startswith('text/html'):
                return render(request, 'setup_questions.html', {'questions': questions, 'uidb64': uidb64, 'token': token})
    ...
    def post(self, request, uidb64, token):
        try:
            ...
            active_status = Status.objects.get(status_name="Active")
...
class SetupPasswordView(views.APIView):
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if request.META.get('HTTP_ACCEPT', '').startswith('text/html'):
                return render(request, 'setup_password.html', {'uidb64': uidb64, 'token': token})
